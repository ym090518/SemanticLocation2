...
def visual_data(png_path, json_data, show_idx):
    ...
    for k in range(len(plotpoints)):
        ...
        plt.savefig('E:/pythonspace/SemanticLocalizationMetrics/test_data/imgs/img_with_annotation/'+str(show_idx)+'.png')
    plt.show()
...
if __name__ == "__main__":
    ...
    # plot one sample
    # visual_data(png_path, json_data, 0)
    print(len(json_data))
    for idx in range(len(json_data)):
        visual_data(png_path, json_data, idx)
        if idx==3 :
            exit()
